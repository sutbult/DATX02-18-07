...
function Ether(rpc = "http://localhost:8545") {
    var currency = new Currency.construct(getEtherBalance, sendEtherContract, validateEtherContract, claimContract, getPastClaim, unlockAccount);
...
async function getPastClaim(contract_address, from_block = 10849){
    ...
    if(events.length == 0){
        ...
        result.claimed = true;
        result.message = events[0].returnValues._hash;
    }
    return result;
}

function claimContract(pre_image_hash, from_address, claim_address){
    var contract;
